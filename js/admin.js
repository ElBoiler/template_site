...
(function () {
  ...
  const COLOUR_DEFS = [
    ...
    { id: 'clr-accent-light', prop: '--clr-accent-light',  label: 'Akzent Hell (Kartenhintergrund)', def: '#F1EFE3' },
    { id: 'clr-text',         prop: '--clr-text',           label: 'Fließtext',                       def: '#333333' },
    { id: 'clr-bg',           prop: '--clr-bg',             label: 'Seitenhintergrund',               def: '#FFFFFF' },
    { id: 'clr-surface',      prop: '--clr-surface',        label: 'Bereichshintergrund (Karten)',    def: '#F8FAFC' },
  ...
  function buildColourGrid() {
    ...
    COLOUR_DEFS.forEach(def => {
      ...
      row.innerHTML =
        `<input type="color" id="picker-${def.id}" class="colour-picker" value="${def.def}">` +
        `<input type="text"  id="hex-${def.id}"    class="colour-hex" maxlength="7" placeholder="${def.def}" value="${def.def}">` +
        `<label for="picker-${def.id}" class="colour-label">${def.label}</label>`;
      const picker = row.querySelector('.colour-picker');
      const hexEl  = row.querySelector('.colour-hex');
  ...
  function handleImageFile(file) {
    ...
    img.onload = () => {
      ...
      Object.entries(derived).forEach(([id, hex]) => setPickerVal(id, hex));

      const thumb = $('themeImgThumbnail'), seedRes = $('themeSeedResult');
      if (thumb) { thumb.onload = () => URL.revokeObjectURL(url); thumb.src = url; }
      if (seedRes) seedRes.hidden = false;
      const swEl = $('themeSwatches');
      if (swEl) swEl.innerHTML = swatches.map(s => `<div class="theme-swatch" style="background:${s.hex}" title="${s.hex}"></div>`).join('');
      applyPreview();
    };
    img.src = url;
  }
  ...
  if (themeImageInput) themeImageInput.addEventListener('change', e => handleImageFile(e.target.files[0]));
  if (themeDropZone) {
